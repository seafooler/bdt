...
class InstanceManager:
    ...
    def _get_ami(self, client):
        ...
        # Print.heading(f'response {response}'),
        # image = 'ami-04cc2b0ad9e30a9c8'
        image = response['Images'][0]['ImageId']
        Print.heading(f'image {image}'),
    ...
    def create_instances(self, instances):
        ...
        try:
            ...
            for client in progress:
                Print.heading(f'self.settings.key_name {self.settings.key_name}'),
                client.run_instances(
                    ImageId=self._get_ami(client),
                    # Print.heading(f'ImageId {ImageId}'),
                    # ImageId="ami-015101ccdf8c4ae32",
                ...
                Print.info(f'create one instance successfully ...'),
    ...
    def start_instances(self, max):
        ...
        try:
            ...
            for region, client in self.clients.items():
                Print.heading(f'Region: {region}, Client: {client}')
                if ids[region]:
                    target = ids[region]
