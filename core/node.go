...
type Client struct {
	Name              string
	CorrespondingNode *gorpc.Client
}
...
type Node struct {
	*config.Config
	...
	payloadMine       map[[HASHSIZE]byte]bool
	committedPayloads map[[HASHSIZE]byte]bool
	proposedPayloads  map[[HASHSIZE]byte]bool
	PayLoadSavings    map[[HASHSIZE]byte]*Mypayload
	LocalSavings      map[[HASHSIZE]byte]*PayLoadMsg
	maxNumInPayLoad   int
...
func NewNode(conf *config.Config) *Node {
	node := &Node{
		...
		payloadMine:       make(map[[HASHSIZE]byte]bool),
		committedPayloads: make(map[[HASHSIZE]byte]bool),
		proposedPayloads:  make(map[[HASHSIZE]byte]bool),
		PayLoadSavings:    make(map[[HASHSIZE]byte]*Mypayload),
		LocalSavings:      make(map[[HASHSIZE]byte]*PayLoadMsg),
...
// StartClientRPC serve for the Client requests
func (n *Node) StartClientRPC() {
	...
	s := &gorpc.Server{
		...
		Handler: func(clientAddr string, payLoad interface{}) interface{} {
			// n.Logger.Info("process payload!")
			assertedPayLoad, ok := payLoad.(*PayLoadMsg)
			if !ok {
				panic("message send is not a payload")
			}
			//n.Logger.Info("Receive Client PayLoad!", "ClientName", n.Client.Name)
			n.Lock()
			defer n.Unlock()
			n.LocalSavings[assertedPayLoad.Hash] = assertedPayLoad
			return nil
		},
	}
	...
	s := &gorpc.Server{
		...
		Handler: func(clientAddr string, payLoad interface{}) interface{} {
			...
			if _, ok := n.committedPayloads[assertedPayLoad.Hash]; ok {
				...
				if assertedPayLoad.Sender == n.Name {
					n.payLoads[assertedPayLoad.Hash] = true
					n.payloadMine[assertedPayLoad.Hash] = true
					n.PayLoadSavings[assertedPayLoad.Hash] = &Mypayload{assertedPayLoad, true}
				} else {
					n.payLoads[assertedPayLoad.Hash] = true
					n.PayLoadSavings[assertedPayLoad.Hash] = &Mypayload{assertedPayLoad, false}
				}
...
func (n *Node) BroadcastClientPayLoadLoop() {
	gorpc.RegisterType(&PayLoadMsg{})
	payLoadFullTime := float32(n.Config.MaxPayloadSize) / float32(n.Config.TxSize*n.Rate)
	n.logger.Debug("payloadFullTime", "s", payLoadFullTime)

	for {
		trans := generateTX(n.Config.TxSize)
		...
		start = time.Now()
		for i := 0; i < txNum; i++ {
			payLoadMsg.Reqs[i] = trans
			payLoadMsg.Reqs[i][n.Config.TxSize-1] = '0'
		}
		n.logger.Debug("2nd step takes", "ms", time.Now().Sub(start).Milliseconds())
		n.BroadcastClientRequest(payLoadMsg)
		//time.Sleep(time.Millisecond * 100)
	}
}

// BroadcastPayLoadLoop mocks the underlying payload broadcast
func (n *Node) BroadcastPayLoadLoop() {
	gorpc.RegisterType(&PayLoadMsg{})

	for {
		n.Lock()
		for _, payLoadMsg := range n.LocalSavings {
			n.BroadcastPayLoad(payLoadMsg, nil)
			delete(n.LocalSavings, payLoadMsg.Hash)
		}
		n.Unlock()
	}
}

func (n *Node) BroadcastClientRequest(data interface{}) {
	go func() {
		start := time.Now()
		if _, err := n.Client.CorrespondingNode.Call(data); err != nil {
			panic(err)
		}
		n.logger.Debug("Sending a payload", "ms", time.Now().Sub(start).Milliseconds(), "id", n.Client.Name)
	}()
}
...
func (n *Node) HandleMsgsLoop() {
	...
	//n.timer.Reset(time.Duration(payLoadFullTime*2000) * time.Millisecond)
	for {
		select {
		case msg := <-msgCh:
			switch msgAsserted := msg.(type) {
...
func (n *Node) createBlock(opt bool) ([][HASHSIZE]byte, int) {
	...
	payLoadCount := len(n.payloadMine)
...
func (n *Node) EstablishClientRPC() {
	addrWithPort := n.Addr + ":" + n.ClientPort
	c := &gorpc.Client{
		Addr:           addrWithPort,
