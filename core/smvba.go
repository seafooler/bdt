...
func (s *SMVBA) HaltOrPreVote(sn, v int, coinNode string, txNum int) {
	...
	if finishMsgByCoin != nil && s.output == nil {
		...
		committedCnt := s.updatePayloads(payLoadHashes, sn)
		...
		s.logger.Info("Commit a block from SMVBA", "replica", s.node.Name, "SN", sn, "View", v,
			...
			"len(payLoadHashes)", len(payLoadHashes), "totalCommittedPayload", len(s.node.committedPayloads))
		go func() {
...
func (s *SMVBA) updatePayloads(payLoadHashes [][HASHSIZE]byte, height int) int {
	committedCount := 0
	s.node.Lock()
	for _, plHash := range payLoadHashes {
		if _, okk := s.node.PayLoadSavings[plHash]; okk {
			if _, ok := s.node.payloadMine[plHash]; ok {
				delete(s.node.payloadMine, plHash)
				delete(s.node.payLoads, plHash)
				s.node.committedPayloads[plHash] = true
				committedCount++
				s.logger.Info("Payload commitInfo:", "height", height,
					"TXnums", s.node.PayLoadSavings[plHash].Info.Count,
					"timestamp", s.node.PayLoadSavings[plHash].Info.TimeStamp)
			} else if _, ok := s.node.payLoads[plHash]; ok {
				delete(s.node.payLoads, plHash)
				s.node.committedPayloads[plHash] = true
				committedCount++
				s.logger.Info("Payload commitInfo:", "height", height,
					"TXnums", s.node.PayLoadSavings[plHash].Info.Count,
					"timestamp", s.node.PayLoadSavings[plHash].Info.TimeStamp)
			} else if _, existed := s.node.committedPayloads[plHash]; existed {
				s.logger.Info("PayLoad has been committed!")
...
func (s *SMVBA) HandleVoteMsg(vm *SMVBAVoteMessage) {
	...
	if len(s.voteMessageMap[vm.View]) == 2*s.node.F+1 {
		...
		if trueCount == 2*s.node.F+1 && s.output == nil {
			...
			committedCnt := s.updatePayloads(payLoadHashes, s.node.sn)
